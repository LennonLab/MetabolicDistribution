...
package.list <- c('BiocManager', 'vegan', 'fitdistrplus', 'ggplot2','here', 'actuar', 'gdata', 'ggpmisc', 'ggpp') 
...
MD_RTLC_11_RSG <- process(here("data", "MetDistRA21", "20210705_RTLC_S2_RSG", "1.75.fcs"), "B530-H")
MD_RTLC_12_RSG <- process(here("data", "MetDistRA21", "20210705_RTLC_S2_RSG", "2.25.fcs"), "B530-H")
MD_RTLC_13_RSG <- process(here("data", "MetDistRA21", "20210705_RTLC_S2_RSG", "2.75.fcs"), "B530-H")
MD_RTLC_14_RSG <- process(here("data", "MetDistRA21", "20210705_RTLC_S2_RSG", "3.25.fcs"), "B530-H")
MD_RTLC_15_RSG <- process(here("data", "MetDistRA21", "20210705_RTLC_S2_RSG", "3.75.fcs"), "B530-H")
MD_RTLC_16_RSG <- process(here("data", "MetDistRA21", "20210705_RTLC_S2_RSG", "4.25.fcs"), "B530-H")
MD_RTLC_17_RSG <- process(here("data", "MetDistRA21", "20210705_RTLC_S2_RSG", "4.75.fcs"), "B530-H")
MD_RTLC_18_RSG <- process(here("data", "MetDistRA21", "20210705_RTLC_S2_RSG", "5.25.fcs"), "B530-H")
MD_RTLC_19_RSG <- process(here("data", "MetDistRA21", "20210705_RTLC_S2_RSG", "5.75.fcs"), "B530-H")
MD_RTLC_20_RSG <- process(here("data", "MetDistRA21", "20210705_RTLC_S2_RSG", "6.25.fcs"), "B530-H")
MD_RTLC_21_RSG <- process(here("data", "MetDistRA21", "20210705_RTLC_S2_RSG", "6.75.fcs"), "B530-H")
MD_RTLC_22_RSG <- process(here("data", "MetDistRA21", "20210705_RTLC_S2_RSG", "7.25.fcs"), "B530-H")
MD_RTLC_23_RSG <- process(here("data", "MetDistRA21", "20210705_RTLC_S2_RSG", "7.75.fcs"), "B530-H")
MD_RTLC_24_RSG <- process(here("data", "MetDistRA21", "20210705_RTLC_S2_RSG", "8.25.fcs"), "B530-H")
...
colnames(MD_RSG) <- c("MD_RTLC_01", "MD_RTLC_02", "MD_RTLC_03", "MD_RTLC_04", "MD_RTLC_05", "MD_RTLC_06", "MD_RTLC_07", "MD_RTLC_08", "MD_RTLC_09", "MD_RTLC_10", "MD_RTLC_11", "MD_RTLC_12", "MD_RTLC_13", "MD_RTLC_14", "MD_RTLC_15", "MD_RTLC_16", "MD_RTLC_17", "MD_RTLC_18", "MD_RTLC_19", "MD_RTLC_20", "MD_RTLC_21", "MD_RTLC_22", "MD_RTLC_23", "MD_RTLC_24")
...
Samples$dist <- c(NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, 0, 25, 50, 75, 100, 125, 150, 175, 200, 225, 250, 275, 300, 325)
...
met_samples$Tau <- c(NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, 2.875, 3.25, 3.875, 4.125, 4.125, 4.625, 4.875, 5.125, 5.375, 5.625, 1.75, 2.25, 2.75, 3.25, 3.75, 4.25, 4.75, 5.25, 5.75, 6.25, 6.75, 7.25, 7.75, 8.25)
```
...
```{r}
hist_MD_RTLC <- ggplot(MD_RTLC_07_RSG, aes(x = RSG_ab, after_stat(density)))+
...
shape <- ggplot(subset(met_samples, met_samples$Set == "RTLC"), aes(x = shape, y = uMChr))+
  geom_point(size = 2, alpha = 0.6)+
  xlab(expression(paste("Shape parameter (", alpha, ")")))+
  ylab(expression(paste("Bacterial Production (", mu, "M C/hr)")))+
  geom_smooth(method = "lm", formula = y~x)+
...
scale <- ggplot(subset(met_samples, met_samples$Set == "RTLC"), aes(x = scale, y = uMChr))+
  geom_point(size = 2, alpha = 0.6)+
  xlab(expression(paste("Scale parameter (", chi[m], ")")))+
  ylab(expression(paste("Bacterial Production (", mu, "M C/hr)")))+
  geom_smooth(method = "lm", formula = y~x)+
...
hist_MD_ULRG <- ggplot(MD_ULRG_14_A, aes(x = RSG_ab, after_stat(density)))+
  geom_histogram(color = "blue", fill = alpha("blue", 0.3), bins = 50)+
  geom_histogram(data = MD_ULRG_06_A, aes(x = RSG_ab, after_stat(density)), color = "red", fill = alpha("red", 0.3), bins = 50)
  xlab("RSG intensity")

hist_MD_ULRG

ggsave("../output/ULRG_met_hist.pdf")
ggsave("../output/ULRG_met_hist.png")
